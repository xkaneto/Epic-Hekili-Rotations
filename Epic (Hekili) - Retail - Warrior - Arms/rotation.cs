...
namespace AimsharpWow.Modules
{
    public class EpicWarriorArmsHekili : Rotation
    {
        ...
        public override void Initialize()
        {
            ...
            Aimsharp.PrintMessage("Epic PVE - Warrior Arms", Color.Yellow);
        ...
        public override bool CombatTick()
        {
            ...
            if (Aimsharp.TargetIsEnemy() && TargetAlive() && TargetInCombat)
            {
                if (Wait <= 200)
                {
                    ...
                    if ((SpellID1 == 324128 || SpellID1 == 376079) && Aimsharp.CanCast(SpearOfBastion_SpellName(Language), "player", false, true))
                    {
                        switch (SpearOfBastionCast)
                        {
                            case "Manual":
                                if (Debug)
                                {
                                    Aimsharp.PrintMessage("Casting Death's Due - " + SpearOfBastionCast + " - Queue", Color.Purple);
                                }
                                Aimsharp.Cast(SpearOfBastion_SpellName(Language));
                                return true;
                            case "Player":
                    ...
                    //Class Spells
                    //Instant [GCD FREE]
                    if (SpellID1 == 118038 && Aimsharp.CanCast(DieByTheSword_SpellName(Language), "target", false, true))
                    {
                    ...
                    if (SpellID1 == 386164 && Aimsharp.CanCast(BattleStance_SpellName(Language), "player", false, true))
                    {
