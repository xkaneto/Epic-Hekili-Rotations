...
namespace AimsharpWow.Modules
{
    public class EpicWarriorProtectionHekili : Rotation
    {
        ...
        #region Lists
        //Lists
        private List<string> m_IngameCommandsList = new List<string> { "NoInterrupts", "NoCycle", "StormBolt","QueueRavager", "ChampionsSpear", "HeroicLeap" };
        ...
        public override bool CombatTick()
        {
            ...
            if (Ravager && CanCastCheck(Ravager_SpellName(Language), "player", false, true))
            {
                switch (RavagerCast)
                {
                    case "Manual":
            ...
            if (Aimsharp.TargetIsEnemy() && TargetAlive() && TargetInCombat)
            {
                if (Wait <= 200)
                {
                    ...
                    //Class Spells
                    //Instant [GCD FREE]
                    if (SpellID1 == 871 && CanCastCheck(ShieldWall_SpellName(Language), "player", false, true))
                    {
                        Aimsharp.Cast(ShieldWall_SpellName(Language), true);
                        return true;
                    }

                    if (SpellID1 == 12975 && CanCastCheck(LastStand_SpellName(Language), "player", false, true))
                    {
                    ...
                    //Revenge
                    if (SpellID1 == 6572 && CanCastCheck(Revenge_SpellName(Language), "target", false, true))
                    {
                    ...
                    if (SpellID1 == 228920 && CanCastCheck(Ravager_SpellName(Language), "player", false, true))
                    {
                        switch (RavagerCast)
                        {
                            case "Manual":
